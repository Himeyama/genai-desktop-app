...
> **注意**: この手順はAWSにデプロイされた本番環境向けです。インフラ（CDK）コードは本リポジトリには含まれていません。
...
インフラリポジトリの CDK パラメーターファイルを確認し、`vpcIdForInvokeExApp` の指定有無を確認します。
...
1. インフラリポジトリの CDK パラメーターファイルで VPC ID を確認
...
1. インフラリポジトリの CDK パラメーターファイルを開く
